Page({
    ...
    onCardSelect(event) {
        ...
        if (isSelected) {
            ...
            newSelectedCards = selectedCards.filter(card => !(card.suit === suit && card.value === value));
        }
    ...
    onPlayCards() {
        ...
        const newMyCards = myCards.filter(card => !selectedCards.some(selected => selected.suit === card.suit && selected.value === card.value));
        // 更新出牌记录
