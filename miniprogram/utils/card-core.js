 ...
 * 创建一张卡牌
 */
export function createCard(suit, value, displayValue) {
 ...
 */
export function sortCards(cards, sortBy = 'value') {
...
export function removeCardsFromHand(hand, cardsToRemove) {
    const cardsToRemoveSet = new Set(cardsToRemove.map(c => `${c.suit}_${c.value}_${c.displayValue}`));
...
export function hasCards(hand, cards) {
    const handSet = new Set(hand.map(c => `${c.suit}_${c.value}_${c.displayValue}`));
 ...
 * 获取手牌中某个点数的所有牌
 */
export function getCardsByValue(hand, value) {
    return hand.filter(c => c.value === value);
}
/**
 * 统计手牌中各点数的数量
 */
export function countCardsByValue(hand) {
 ...
 * 统计手牌中各花色的数量
 */
export function countCardsBySuit(hand) {
    const countMap = new Map();
    for (const card of hand) {
 ...
 * 创建牌堆对象
 */
export function createDeckObject(cards) {
    return {
        cards,
