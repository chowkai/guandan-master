 ...
 * 检查同花顺
 */
function checkSameColorStraight(cards: Card[]): HandType | null {
 ...
 * 检查钢板（2 个连续三张）
 */
function checkSteelPlate(cards: Card[]): HandType | null {
 ...
 * 检查连对（3 对连续）
 */
function checkConsecutivePairs(cards: Card[]): HandType | null {
 ...
 * 检查顺子（5 张连续）
 */
function checkStraight(cards: Card[]): HandType | null {
  // 只取前 5 张检查
 ...
 * 检查三带二
 */
function checkTripleWithPair(cards: Card[]): HandType | null {
  const valueCount = countCardsByValue(cards);
 ...
 */
export function compareHands(hand1: HandType, hand2: HandType): number {
  // 首先比较牌型优先级
 ...
 * 获取牌型名称（中文）
 */
export function getHandTypeName(type: CardType): string {
 ...
 * 获取牌型的牌数要求
 */
export function getHandTypeLength(type: CardType): number | null {
  const lengths: Record<CardType, number | null> = {
    [CardType.SINGLE]: 1,
    [CardType.PAIR]: 2,
    [CardType.TRIPLE]: 3,
